...
namespace POExileDirection
{
    public partial class NinjaForm : Form
    {
        ...
        {
            ninjaData = null;
            ninjaData = new NinJaAPIData();
            CurrentLeague = (string)cbLeague.SelectedItem;
            textBox1.Text = "";
            textBox2.Text = "";
            textBoxChaos.Text = "";
            textBoxExalted.Text = "";
        ...
        private void GetNinJaDataSync()
        {
            ...
            /*var taskGetData = GetJsonData(CurrentLeague);
            if (taskGetData.Status == TaskStatus.Faulted)
            {
                MessageBox.Show("데이터를 가져오는데 문제가 있습니다. 다시 시도해주세요.", "DeadlyCrush", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            ...
            var taskUpdateData = UpdatePoeNinjaData();
            if(taskUpdateData.Status == TaskStatus.Faulted)
            {
                MessageBox.Show("데이터를 업데이트하는데 문제가 있습니다. 다시 시도해주세요.", "DeadlyCrush", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            while (!taskGetData.IsCompleted)
            {
                panelGetData.Visible = true;
            ...
            if (UpdatingFromAPI || UpdatingFromJson)
            {
                // Done
                panelGetData.Visible = false;
                labelGetData.Visible = false;
        ...
        public void Init_Controls()
        {
            ...
            cbLeague.SelectedIndex = cbLeague.FindStringExact("Legion");
        }
        #endregion
        ...
        private void GetJsonData(string league) // async Task
        {
            // RM_2019_0713 Task tWait =
            /*await Task.Run(() =>
            {
            ...
            // RM_2019_0713 tWait.Wait();
        }

        private void UpdatePoeNinjaData() // async Task
        {
            // RM_2019_0713 Task tWait =
            /*await Task.Run(() =>
            {
